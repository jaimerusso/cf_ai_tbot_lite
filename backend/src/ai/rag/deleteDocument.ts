...
export class DeleteDocumentWorkflow extends WorkflowEntrypoint<Env, Params> {
	async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
		...
		console.log(name, ' - Step 1: Update the document status to deleting');
		...
		console.log(name, ' - Step 2: Get the embeddings ids from the DO');
		...
		console.log(name, ' - Step 3: Delete the document embedding(s) from the vector');
		...
		console.log(name, ' - Step 4: Generate new search documents tool description');
		...
		console.log(name, ' - Step 5: Await vector deletion and then delete the document from the DO');
		...
		//Finished
		console.log(name, ' - Delete Document Workflow completed\n\n');
	}
}

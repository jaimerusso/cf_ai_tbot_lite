...
export class SearchWorkflow extends WorkflowEntrypoint<Env, Params> {
	async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
		...
		console.log('Step 1: Generate vector embedding for the search query');
		...
		console.log('Step 2: Search for top 3 similar vectors in the vector database');
		const matches = await step.do(`search-vectors`, async () => {
			return await this.env.VECTORIZE.query(embedding, {
				topK: 3,
				returnValues: true,
				returnMetadata: 'all',
			});
